...
const compactTypographyScale = 0.9;

function scaleTypography(typography: PreviewTypography, scale: number): PreviewTypography {
  return {
    fullName: Math.round(typography.fullName * scale),
    jobTitle: Math.round(typography.jobTitle * scale),
    contact: Math.round(typography.contact * scale),
    sectionTitle: Math.round(typography.sectionTitle * scale),
    sectionItemTitle: Math.round(typography.sectionItemTitle * scale),
    sectionDetails: Math.round(typography.sectionDetails * scale),
  };
}
...
export default function ResumePreview({
  ...
  const renderedTypography = isCompact ? scaleTypography(typography, compactTypographyScale) : typography;
  const shellClass = isModern ? 'bg-slate-950 text-white' : 'bg-white text-slate-900';
  const pageClass = isCompact ? 'mx-auto w-full max-w-[920px] px-3 py-4 lg:px-4 lg:py-5' : 'mx-auto w-full max-w-[1100px] px-4 py-7 lg:px-6 lg:py-8';
      ...
      >
        ...
        <header className={`pb-5 ${mode === 'ats' ? 'border-b border-black' : isModern ? 'border-b border-white/20' : 'border-b border-slate-200'}`}>
          <div className="grid gap-5 lg:grid-cols-[1.15fr_0.85fr] lg:items-start">
            <div className="flex min-w-0 gap-4">
              <div className="flex min-w-0 flex-1 flex-col gap-0.5">
                <h1
                  ...
                  style={{ fontSize: `${renderedTypography.fullName}px` }}
                >
                ...
                <p
                  ...
                  style={{ fontSize: `${renderedTypography.jobTitle}px` }}
                >
            ...
            <div className={`leading-[1.55] ${mode === 'ats' ? 'text-black' : isModern ? 'text-white/80' : 'text-slate-500'}`} style={{ fontSize: `${renderedTypography.contact}px` }}>
            ...
            <div className={`mt-5 grid gap-3 text-sm ${mode === 'ats' ? 'text-black' : isModern ? 'text-white/90' : 'text-slate-700'}`}>
              <div>
                <h2
                  ...
                  style={{ fontSize: `${renderedTypography.sectionTitle}px` }}
                >
                  Professional Summary
                </h2>
                <p className={`mt-1 ${isCompact ? 'leading-7' : 'leading-8'}`} style={{ fontSize: `${renderedTypography.sectionDetails}px` }}>
        ...
        <div className={isCompact ? 'mt-3 space-y-5' : 'mt-6 space-y-8'}>
...
function PreviewSection({
  ...
  return (
    <section className="break-inside-avoid resume-section pdf-avoid-break">
      <h2
      ...
      <div className={isCompact ? 'mt-3' : 'mt-4'}>
        {sectionId === 'summary' && (
          <p className={`whitespace-pre-line ${isCompact ? 'leading-7' : 'leading-8'} ${mode === 'ats' ? 'text-black' : isModern ? 'text-white/90' : 'text-slate-700'}`} style={{ fontSize: `${renderedTypography.sectionDetails}px` }}>
            {source.personalInfo.summary}
          </p>
        )}
        {sectionId === 'experience' && (
          <div className={isCompact ? 'space-y-3.5' : 'space-y-5'}>
            {source.experience.map((item) => (
              <article key={item.id} className="break-inside-avoid pdf-avoid-break">
                <div className="flex flex-wrap items-baseline justify-between gap-2">
                  <div>
                    <h3 className={`font-semibold ${mode === 'ats' ? 'text-black' : isModern ? 'text-white' : 'text-slate-900'}`} style={{ fontSize: `${renderedTypography.sectionItemTitle}px` }}>
                      {item.role || 'Role title'}
                    </h3>
                    <p className={`mt-1 ${mode === 'ats' ? 'text-black' : isModern ? 'text-white/80' : 'text-slate-600'}`} style={{ fontSize: `${renderedTypography.sectionDetails}px` }}>
                      {experienceSummary(item) || 'Company'}
                    </p>
                  </div>
                  <p className={`${mode === 'ats' ? 'text-black' : isModern ? 'text-white/70' : 'text-slate-500'}`} style={{ fontSize: `${renderedTypography.sectionDetails}px` }}>
                    {formatRange(item.startDate, item.endDate, item.isCurrent)}
                  </p>
                </div>
                {(item.technologies.length > 0 || item.achievements.length > 0) && (
                  <div className={isCompact ? 'mt-2.5 space-y-1.5' : 'mt-3 space-y-1.5'}>
                    ...
                    <ul className={`list-disc ${isCompact ? 'space-y-1 pl-4' : 'space-y-2 pl-6'} ${mode === 'ats' ? 'text-black marker:text-black' : isModern ? 'text-white/90 marker:text-white/90' : 'text-slate-700 marker:text-brand-600'}`} style={{ fontSize: `${renderedTypography.sectionDetails}px`, lineHeight: isCompact ? 1.5 : 2 }}>
                      {item.achievements.map((bullet, index) => (
                        <li key={index} className={isCompact ? 'break-inside-avoid pdf-avoid-break hyphens-auto break-words' : undefined}>
                          {bullet}
                        </li>
                      ))}
                    </ul>
          ...
          <div className={isCompact ? 'space-y-3.5' : 'space-y-5'}>
            {source.education.map((item) => (
              <article key={item.id} className="break-inside-avoid pdf-avoid-break">
                ...
                {item.description && <p className={`mt-2.5 ${isCompact ? 'leading-7' : 'leading-8'} ${mode === 'ats' ? 'text-black' : isModern ? 'text-white/90' : 'text-slate-700'}`} style={{ fontSize: `${renderedTypography.sectionDetails}px` }}>{item.description}</p>}
              </article>
            ))}
          </div>
        )}
        {sectionId === 'skills' && <p className={`${isCompact ? 'leading-7' : 'leading-8'} ${mode === 'ats' ? 'text-black' : isModern ? 'text-white/90' : 'text-slate-700'}`} style={{ fontSize: `${renderedTypography.sectionDetails}px` }}>{source.skills.join(' · ')}</p>}
        {sectionId === 'projects' && (
          <div className={isCompact ? 'space-y-3.5' : 'space-y-5'}>
            {source.projects.map((item) => (
              <article key={item.id} className="break-inside-avoid pdf-avoid-break">
                ...
                {item.description && <p className={`mt-2.5 ${isCompact ? 'leading-7' : 'leading-8'} ${mode === 'ats' ? 'text-black' : isModern ? 'text-white/90' : 'text-slate-700'}`} style={{ fontSize: `${renderedTypography.sectionDetails}px` }}>{item.description}</p>}
                {item.link && (
                  <p className={`mt-2 ${mode === 'ats' ? 'text-black' : isModern ? 'text-white/80' : 'text-slate-700'}`} style={{ fontSize: `${renderedTypography.sectionDetails}px` }}>
        ...
        {sectionId === 'languages' && (
          <div className={`flex flex-wrap ${isCompact ? 'gap-1.5' : 'gap-2'} ${mode === 'ats' ? 'text-black' : isModern ? 'text-white/90' : 'text-slate-700'}`} style={{ fontSize: `${renderedTypography.sectionDetails}px` }}>
            {source.languages.map((item) => (
              <span key={item.id} className={`rounded-full ${isCompact ? 'px-2.5 py-1' : 'px-3 py-1.5'} ${mode === 'ats' ? 'border border-black bg-white' : isModern ? 'border border-white/20 bg-white/5' : 'bg-slate-100'}`}>
                {item.name} — {item.proficiency}
              </span>
            ))}
          </div>
        )}
        {sectionId === 'certifications' && renderSimpleItems(source.certifications, mode, isModern, typography, isCompact)}
        {sectionId === 'volunteer' && renderSimpleItems(source.volunteer, mode, isModern, typography, isCompact)}
        {sectionId === 'awards' && renderSimpleItems(source.awards, mode, isModern, typography, isCompact)}
        {sectionId === 'references' && renderSimpleItems(source.references, mode, isModern, typography, isCompact)}
        {sectionId === 'interests' && (
          <div className={`flex flex-wrap ${isCompact ? 'gap-1.5' : 'gap-2'} ${mode === 'ats' ? 'text-black' : isModern ? 'text-white/90' : 'text-slate-700'}`} style={{ fontSize: `${renderedTypography.sectionDetails}px` }}>
            {source.interests.map((interest) => (
              <span key={interest} className={`rounded-full ${isCompact ? 'px-2.5 py-1' : 'px-3 py-1.5'} ${mode === 'ats' ? 'border border-black bg-white' : isModern ? 'border border-white/20 bg-white/5' : 'bg-slate-100'}`}>
                {interest}
              </span>
            ))}
          </div>
...
function renderSimpleItems(
  ...
  return (
    <div className={isCompact ? 'space-y-2.5' : 'space-y-3'}>
      {items.map((item) => (
        <div key={item.id} className="break-inside-avoid pdf-avoid-break">
          <h3 className={`font-semibold ${mode === 'ats' ? 'text-black' : isModern ? 'text-white' : 'text-slate-900'}`} style={{ fontSize: `${renderedTypography.sectionItemTitle}px` }}>
            {item.title}
          </h3>
          <p className={`mt-1 ${isCompact ? 'leading-7' : 'leading-8'} ${mode === 'ats' ? 'text-black' : isModern ? 'text-white/90' : 'text-slate-700'}`} style={{ fontSize: `${renderedTypography.sectionDetails}px` }}>
            {item.details}
          </p>
        </div>
      ))}
