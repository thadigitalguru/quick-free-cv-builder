...
export const validateDocument = (document: CVDocument): ValidationIssue[] => {
  ...
  if (document.experience.length === 0) {
    issues.push({ field: 'experience', sectionId: 'experience', message: 'Add at least one experience entry.' });
  }

  if (document.education.length === 0) {
    issues.push({ field: 'education', sectionId: 'education', message: 'Add at least one education entry.' });
  }

  if (document.skills.length === 0) {
    issues.push({ field: 'skills', sectionId: 'skills', message: 'Add at least one skill.' });
  }
